...
namespace MyRender
{
    ...
    GLuint LoadTextureFromAssImp(const aiTexture *aiTex, GLint wrapMode, GLint MagFilterMode, GLint MinFilterMode);
    ...
    private:
        ...
        Mesh processMesh(aiMesh *mesh, const aiScene *scene, string const &path)
        {
            // data to fill
            ...
            for (unsigned int i = 0; i < mesh->mNumVertices; i++)
            {
                ...
                if (mesh->HasNormals())
                {
                    ...
                    cout << "normal" << vector.x << vector.y << vector.z << endl;
                }
                // texture coordinates
            ...
            return Mesh(vertices, indices, textures, scene, path);
        }
            ...
            for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
            {
                ...
                if (!skip)
                {
                    ...
                    if (aitexture != nullptr)
                        tex.id = LoadTextureFromAssImp(aitexture, GL_REPEAT, GL_LINEAR, GL_LINEAR);
                    else
                    {
                        auto filePath = directory + '/' + string(str.C_Str());
